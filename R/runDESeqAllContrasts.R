plot_Volcano <- function(r,title, p.cutoff = 0.1, fc.cutoff = 2, top.n = 30) {
  ...
  padj$labels <- ifelse(padj$g_symbol %in% head(diff.ex[order(diff.ex$padj), 
                                                        "g_symbol"], top.n), 
                        padj$g_symbol, NA)
...
get.diff.genes.list <- function(res, p.cutoff = 0.1, fc.cutoff = 2) {
...
plot_heatmap <- function(dg.list,title,dds,annotation.col) {
  ...
  p <- pheatmap::pheatmap(SummarizedExperiment::assay(vsd)[rownames(vsd) %in% dg.list,], cluster_rows=F, show_rownames=FALSE,
                cluster_cols=T, annotation_col = df, main = title)
}

get.all.results <- function(i,dds, mode = "lfcShrink", alpha = 0.1) {
...
#' @export
get.all.MA.plots <- function(res,coef,p.cutoff = 0.1) {
  MA <- DESeq2::plotMA(res, ylim=c(-3,3), main = coef,alpha = p.cutoff)
}

write.diff.genes <- function(res,dg,coef, folder) {
...
#'
#' @export
run.DESeq.all.contrasts <- function(dds,folder, 
                                    mode = "lfcShrink", 
                                    condition,
  ...
  if (!is.numeric(p.cutoff) | !is.numeric(fc.cutoff) | !is.wholenumber(top.n)) {
  ...
  for (g in groups) {
    ...
    par(mfrow = c(3,2))
    mapply(get.all.MA.plots,resList,coef, MoreArgs = list(p.cutoff = p.cutoff))
    volcList <- mapply(plot_Volcano,resList,coef,
                       MoreArgs = list(p.cutoff = p.cutoff, 
                                       fc.cutoff = fc.cutoff, 
                                       top.n = top.n), 
                       SIMPLIFY = F)
    print(volcList)
    dgList <- lapply(resList, get.diff.genes.list, 
    ...
    dev.off()
    mapply(write.diff.genes,resList,dgList,coef, MoreArgs = list(folder = folder))
